...
`
	...
	if p.CustomProxies[0].Type != "socks5" || p.CustomProxies[0].Name != "EXIT" {
		t.Fatalf("custom proxy=%+v", p.CustomProxies[0])
	}
	if len(p.ProxyChains) != 2 {
		t.Fatalf("proxy chains=%d, want=2", len(p.ProxyChains))
	}
	if p.ProxyChains[0].Type != "regex" || p.ProxyChains[0].Regex == nil {
		t.Fatalf("proxy chain0=%+v", p.ProxyChains[0])
	}
	if p.ProxyChains[1].Type != "group" || p.ProxyChains[1].Group != "PROXY" {
		t.Fatalf("proxy chain1=%+v", p.ProxyChains[1])
	}
}
...
version: 1
template:
  quanx: "https://example.com/base.conf"
custom_proxy:
  - name: EXIT
    type: socks5
    server: 1.2.3.4
...
func TestParseProfileYAML_ProxyChainUnsupportedShadowrocket(t *testing.T) {
	yml := `
version: 1
template:
  shadowrocket: "https://example.com/base.conf"
custom_proxy:
  - name: EXIT
    type: socks5
    server: 1.2.3.4
...
rule:
  - "MATCH,DIRECT"
`
	_, err := ParseProfileYAML("https://example.com/profile.yaml", yml, "shadowrocket")
	var pe *ParseError
	if !errors.As(err, &pe) {
		t.Fatalf("expected *ParseError, got %T: %v", err, err)
	}
