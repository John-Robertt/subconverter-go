...
func TestCompile_ProxyChain_UsesMatchNameAndEmitsUsedCustomProxy(t *testing.T) {
	subs := []model.Proxy{
		{Type: "ss", Name: "HK", Server: "hk1.example.com", Port: 1, Cipher: "aes-128-gcm", Password: "pass"},
		{Type: "ss", Name: "HK", Server: "hk2.example.com", Port: 2, Cipher: "aes-128-gcm", Password: "pass"},
		{Type: "ss", Name: "SG", Server: "sg.example.com", Port: 3, Cipher: "aes-128-gcm", Password: "pass"},
	}
	prof := &profile.Spec{
		Version:       1,
		CustomProxies: []model.Proxy{{Name: "EXIT", Type: "socks5", Server: "1.2.3.4", Port: 1080}},
		Groups:        []profile.GroupSpec{{Raw: "PROXY`select`(HK|SG)", Name: "PROXY", Type: "select", RegexRaw: "(HK|SG)", Regex: regexp.MustCompile("(HK|SG)")}},
		ProxyChains:   []profile.ChainSpec{{Raw: "type=regex via=EXIT pattern=^HK$", Type: "regex", Via: "EXIT", Pattern: "^HK$", Regex: regexp.MustCompile("^HK$")}},
	...
	if got.Proxies[0].Name != "EXIT" || got.Proxies[0].Type != "socks5" {
		t.Fatalf("custom proxy=%+v", got.Proxies[0])
	}
	if got.Proxies[1].Name != "HK" || got.Proxies[2].Name != "HK-2" || got.Proxies[3].Name != "SG" {
		t.Fatalf("subscription proxy names=%q,%q,%q", got.Proxies[1].Name, got.Proxies[2].Name, got.Proxies[3].Name)
	}
	if got.Proxies[1].ViaProxyID != got.Proxies[0].ID || got.Proxies[2].ViaProxyID != got.Proxies[0].ID {
		t.Fatalf("HK proxies should both use EXIT: %+v %+v", got.Proxies[1], got.Proxies[2])
	}
	if got.Proxies[3].ViaProxyID != "" {
		t.Fatalf("SG should not use EXIT: %+v", got.Proxies[3])
	}
}

func TestCompile_ProxyChain_Conflict(t *testing.T) {
	subs := []model.Proxy{{Type: "ss", Name: "HK", Server: "hk.example.com", Port: 1, Cipher: "aes-128-gcm", Password: "pass"}}
	prof := &profile.Spec{
		Version: 1,
		CustomProxies: []model.Proxy{
			{Name: "EXIT1", Type: "socks5", Server: "1.2.3.4", Port: 1080},
			{Name: "EXIT2", Type: "http", Server: "5.6.7.8", Port: 8080},
		},
		Groups: []profile.GroupSpec{{Raw: "PROXY`select`[]@all", Name: "PROXY", Type: "select", Members: []string{"@all"}}},
		ProxyChains: []profile.ChainSpec{
			{Raw: "type=all via=EXIT1", Type: "all", Via: "EXIT1"},
			{Raw: "type=regex via=EXIT2 pattern=HK", Type: "regex", Via: "EXIT2", Pattern: "HK", Regex: regexp.MustCompile("HK")},
		},
		Rules: []model.Rule{{Type: "MATCH", Action: "PROXY"}},
	}
	...
	if ce.AppError.Code != "CHAIN_CONFLICT" {
		t.Fatalf("code=%q, want=%q", ce.AppError.Code, "CHAIN_CONFLICT")
	}
}
...
func TestCompile_ProxyChain_GroupCycle(t *testing.T) {
	...
	prof := &profile.Spec{
		...
		ProxyChains: []profile.ChainSpec{{Raw: "type=group via=EXIT group=A", Type: "group", Via: "EXIT", Group: "A"}},
