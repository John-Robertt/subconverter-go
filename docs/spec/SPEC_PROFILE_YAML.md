...
profile 不直接消费 `proxyID`；策略组编译输入仍然是“组名 / 内置动作 / `@all` / 节点展示名正则”。
...
- `<REGEX>`：Go RE2 正则；用于从订阅节点的最终展示名 `Proxy.Name` 中筛选成员（仅节点，不包含其它策略组与 DIRECT/REJECT）。

约束：
- 至少要有 1 个成员（`[]...`）。
- 显式成员列表当前聚焦“组名 / 内置动作 / `@all`”三类语义，不支持直接引用单个订阅节点；若需要按节点选取，请使用正则写法。
- 使用 `<REGEX>` 写法时，筛选结果不能为空（否则错误；避免生成“空组”）。
- `@all` 在编译阶段展开为全部节点的内部引用；展开顺序按《输出稳定性与规范化规范》。
...
说明：
- `<REGEX>`：Go RE2 正则，用于从订阅节点的最终展示名 `Proxy.Name` 中筛选成员
