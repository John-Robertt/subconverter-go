...
- 策略组成员顺序：
  - `select`（显式成员列表）：按 profile 中 `[]成员` 的出现顺序输出；若出现 `@all`，则在该位置展开为“全部节点（按 5.1 的订阅合并顺序）”。
  - `select`（正则筛选写法）：成员来自正则筛选，输出顺序为“订阅合并顺序”（与 5.1 一致）。
  - `url-test`：成员来自正则筛选，输出顺序为“订阅合并顺序”（与 5.1 一致）。
